...
public class Lottery {
    ToyList currentToys;
    MemberQueue currentParticipants;
    double lossWeight = 0;
    int lossId;

    ChanceCalc cc = new ChanceCalc();
    Lottery.QuantityCalc qc = new Lottery.QuantityCalc();

    public Lottery(MemberQueue kids, ToyList tl) {
    ...
    void showRoll(Participant kid, Toy prize, double roll) {
        System.out.printf("%s бросает на %.3f , это ниже шанса %.2f у %s%n", kid.getName(), roll, prize.getChance(), prize.name);
    }
    ...
    class QuantityCalc {
        ...
        PriorityQueue<Toy> adjustQuantityLeft(Toy t, ToyList tl,PriorityQueue<Toy> currentQueue) {
            ...
            if(t.quantity == 0){
                removeStock(t.id, tl);
                Lottery.this.cc.assignChance(tl);
