//Промежуточная контрольная работа по блоку
//специализация
//Место в программе
//Дается после блока программирования и курсов “Знакомство с языком
//программирования Python” и “Java: знакомство и как пользоваться
//базовыми API”.
//Информация о работе
//Включает в себя два задания: “Приложение заметки” – Урок 1, которое
//выполняется на ЯП Python и “Магазин игрушек” – Урок 2, которое
//выполняется на ЯП “Java”.
//Задание 2. Магазин игрушек (Java)
//Информация о проекте
//Необходимо написать проект, для розыгрыша в магазине игрушек. Функционал
//должен содержать добавление новых игрушек и задания веса для выпадения
//игрушек.
//Как сдавать проект
//Для сдачи проекта необходимо создать отдельный общедоступный
//репозиторий(Github, gitlub, или Bitbucket). Разработку вести в этом
//репозитории, использовать пул реквесты на изменения. Программа должна
//запускаться и работать, ошибок при выполнении программы быть не должно.
//Программа, может использоваться в различных системах, поэтому необходимо
//разработать класс в виде конструктора
//Задание
//1) Напишите класс-конструктор у которого принимает минимум 3 строки,
//содержащие три поля id игрушки, текстовое название и частоту выпадения
//игрушки
//2) Из принятой строки id и частоты выпадения(веса) заполнить минимум три
//массива.
//3) Используя API коллекцию: java.util.PriorityQueue добавить элементы в
//коллекцию
//4) Организовать общую очередь 5) Вызвать Get 10 раз и записать результат в
//файл
//Подсказка:
//В метод put передаете последовательно несколько строк
//1 2 конструктор;
//2 2 робот;
//3 6 кукла.
//Метод Get должен случайно вернуть либо “2”, либо “3” и соответствии с весом.
//В 20% случаях выходит единица
//В 20% двойка
//И в 60% тройка.
//Критерии оценки
//Приложение должно запускаться, записывать значения в файл

import java.util.List;

public class Main {
    public static void main (String[] args){

        ToyList toys1 = new ToyList();
        toys1.addToy(new Toy(23,"плюшевый кролик", 4));
        toys1.addToyList(List.of(
                new Toy(6, "Самокат",2),
                new Toy(11, "Дженга",3),
                new Toy(9, "Фишки",1)
        ));
        System.out.println(toys1);


        MemberQueue line = new MemberQueue(List.of(
                new Participant("Роман"),
                new Participant("Светлана"),
                new Participant("Георгий"),
                new Participant("Максим"),
                new Participant("Юлия"),
                new Participant("Ольга"),
                new Participant("Алексей"),
                new Participant("Семен"),
                new Participant("Надежда"),
                new Participant("Валентина")
        ));


        Lottery lott = new Lottery(line, toys1);
        System.out.println(lott.currentToys.toString());
        lott.runLottery();


        System.out.println("\nРозыгрышь с возможностью проиграть");
        MemberQueue lineloose = new MemberQueue();
        for (int i = 1; i <= 10; i++) {
            lineloose.addMember(new Participant());
        }

        Lottery lott2 = new Lottery(lineloose, toys1);
        lott2.setLossWeight(30);
        System.out.println(lott2.currentToys.toString());
        lott.runLottery();


        toys1.saveToFile();


    }
}
